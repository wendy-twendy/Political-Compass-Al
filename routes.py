...
from models import questions, municipalities
    ...
    try:
        ...
        for i, answer in enumerate(answers):
            if not isinstance(answer, (int, float)) or answer < 1 or answer > 5:
                return jsonify({
                    "error": f"Invalid answer for question {i+1}: must be between 1 and 5"
                }), 400
        ...
        comparisons = {
            'average_scores': {
                'overall': {'economic': 2.5, 'social': -1.5},
                'age_group': {'economic': 3.0, 'social': -2.0},
                'sex': {'economic': 1.5, 'social': -1.0},
                'country': {'economic': 2.0, 'social': -1.8}
            },
            'famous_figures': {
                'Figure 1': {'economic': -3.0, 'social': 4.0},
                'Figure 2': {'economic': 6.0, 'social': -2.0},
                'Figure 3': {'economic': -1.0, 'social': 0.0}
            }
        }
...
def shared_result():
    ...
    quadrant, explanation = get_quadrant_explanation(economic, libertarian_authoritarian)
